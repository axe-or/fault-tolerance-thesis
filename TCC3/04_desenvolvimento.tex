...
% TODO: Referenciar o quadro de DeadlineWatcher
...
Apesar do alvo de compilação do projeto ser um microcontrolador com o FreeRTOS com CMSISv2, o design transparente permitiu também a implementação para testes e um ciclo de debug mais rápido em sistemas operacionais completos (Linux e Windows 10/11) sem necessitar de um simulador. Os detalhes de plataforma são consolidados em uma única unidade de tradução e condicionalmente compilados dependendo do sistema alvo. Todo código específico de plataforma pode ser encontrado em suas respectivas unidades \texttt{platform\_PLATAFORMA.cpp}. A versão de teste utiliza de alguns primitivos de semáforo do FreeRTOS, porém esses podem ser generalizados para multi-plataforma.

\subsection{Programa de Teste}

O programa de teste escolhido foi desenvolvido em 3 versões, o algoritmo de convolução é idêntico, tendo como alteração apenas o setup das tarefas adicionais (no caso de TMR) e a alocação inicial de memória.

% TODO: foto do gato e do lado foto do gato sobel

O fluxo do programa de teste pode ser generalizado da seguinte forma:

% TODO: Fluxo grama linha linha

\section{Verificação}

O primeiro passo após compilar o projeto em um arquivo ELF com informação de debug é encontrar quais os símbolos seus endereços para relizar a injeção. Utilizando um script Python com a biblioteca \texttt{elftools} para analisar a tabela de símbolos do arquivo. É também possível 

% TODO: Imagem dos simbolos

Foram selecionados os símbolos globais:
- task\_arena: Ponteiro base das alocações do stack das tarefas, será utilizada para corrupção de stack aleatória
- input: Pedaço da imagem de input decodificada
- output: Espaço para a imagem de output

Para os símbolos locais, é necessário identificar seu offset de pilha no momento da execução, para isso será feita inspeção ad-hoc no depurador. Os alvos escolhidos foram os filtros da imagem, pois possuem uma área maior e resultam em impacto significativo no output quando modificados.

São sorteados 2 deslocamentos para as corrupções na pilha. É feito o flash do programa de teste na placa e inicia-se uma sessão de debugging no ST-Link.
...
% O programa sempre será pausado logo após o setup inicial das arenas e tasks e antes da saída, são realizados 2 rodadas de injeção, uma com a injeção em pontos conhecidos e outra para os offsets aleatórios.

% As falhas fixas (input, output e filtro) são injetadas durante 3 iterações da convolução, na linha 30 e na linha 60 e 90 da imagem, que correspondem à aproximadamente um terço do processo. As falhas aleatórias são injetadas nas iterações 40 e 80 da convolução, mas afetam um pedaço arbitrário de pilha.

% % TODO: Imagem no VScode(debugging)

% A injeção é aplicada através da janela de símbolos:

% % TODO: Imagem no VScode(simb)

% O programa é resumido e seu comportamento monitorado, antes de sair as métricas coletadas são lidas e armazenadas. 
% Caso ocorra uma violação de deadline, será possível notar um salto para o \texttt{HardFault\_Handler} assim como uma mensagem no console indicando qual tarefa violou caso seja possível identificá-la.

% O output do programa é emitido pela conexão VirtualCOM e é colocado num arquivo \texttt{out.pgm} redirecionando o resultado da conexão serial com um programa de linha de comando da seguinte forma: \texttt{microcom -p /dev/ttyACM0 > out.pgm}.

% % TODO: Imagem do VSCode e o arquivo resultado do lado
