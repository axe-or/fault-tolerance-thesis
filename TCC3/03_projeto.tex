...
Serão injetadas por caso até 5 falhas transientes. Os tipos de falha serão: 3 falhas com endereços fixos, com o objetivo de corromper variáveis locais ou globais já conhecidas com o fim de causar falhas em pontos do programa que produzam resultados diretos, e 2 falhas que utilizarão de um endereço base e causarão uma corrupção aleatória a partir do endereço base.

As regiões de memória escolhidas estarão no segmento estático com escrita e nos stack frames das tarefas. O programa de teste será executado com as mesmas falhas porém com diferentes combinações de técnicas.
...
Para testar a viabilidade e o impacto das técnicas implementadas, será utilizado um programa de teste que lerá uma imagem e aplicará um filtro de detecção de bordas Sobel utilizando convolução, o resultado é salvo num arquivo após ser enviado via VirtualCOM.
