...
Foram selecionadas execuções que resultam em um output que possa ser visualizado para demonstrar o efeito dos upsets no resultado final. A  \autoref{fig:catSobel} demonstra o output desejado, com a imagem de um gato com um filtro Sobel aplicado. A \autoref{fig:catSobelKernelUpset} mostra o impacto de um upset causado no filtro (kernel) da convolução. Na \autoref{fig:catSobelSubtleUpset} a corrupção é bem mais sutil, manifestando-se como uma pequena horizontal clara na linha 6. Por fim, a figura \autoref{fig:catSobelInterrupted} demonstra o efeito da convolução sendo interrompida devido à um assert, deixando o resultado incompleto.
