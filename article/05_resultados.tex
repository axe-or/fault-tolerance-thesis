...
Com o microcontrolador conectado via ST-Link e com VirtualCOM por USB é inicializada uma sessão de depuração, o primeiro passo é inicializar a sessão com o breakpoint padrão na função \texttt{main} (\autoref{fig:injStartDebugger}) que é definida pelo código de plataforma da HAL fornecida pelo fabricante do microcontrolador. É inserido um breakpoint na função \texttt{entrypoint} que o ponto de entrada real da aplicação que ocorre após o setup de periféricos e do RTOS.

\begin{figure}[H]
    \centering
    \captionsetup{justification=centering}
    \caption{Inicializar sessão de depuração no ST-Link}
	\figuraborda{width=0.7\textwidth}{assets/inj_start_debugger.png }
...
\begin{figure}[H]
	...
	\figuraborda{width=0.6\textwidth}{assets/inj_connect_com.png }
    \captionsetup{justification=raggedright}
    \caption*{Fonte: Elaborada pelo autor}
...
\begin{figure}[H]
	...
	\figuraborda{width=0.98\textwidth}{assets/inj_cond_breakpoint.png }
    \captionsetup{justification=raggedright}
    \caption*{Fonte: Elaborada pelo autor}
...
\begin{figure}[H]
	...
	\figuraborda{width=0.85\textwidth}{assets/inj_cause_upset.png }
    \captionsetup{justification=raggedright}
    \caption*{Fonte: Elaborada pelo autor}
    \label{fig:injCauseUpset}
\end{figure}

Após feitas as injeções desejadas, o programa é resumido e seu resultado inspecionado. Neste caso, como demonstrado na \autoref{fig:injResultadoVirtualcom}, o erro foi mascarado via consenso majoritário.

\begin{figure}[H]
    \centering
    \captionsetup{justification=centering}
    \caption{Resultado no VirtualCOM}
	\figuraborda{width=0.65\textwidth}{assets/inj_resultado_virtualcom.png }
    \captionsetup{justification=raggedright}
    \caption*{Fonte: Elaborada pelo autor}
    \label{fig:injResultadoVirtualcom}
\end{figure}

Para observar o resultado, basta definir a macro \texttt{DUMP\_BITMAP} e redirecionar a saída do microCOM para um arquivo PGM. Em um sistema POSIX seria algo como \texttt{microcom -f -p /dev/ttyACM0 > out.pgm}

Refazendo o processo acima com a saída redirecionada é possível observar o output da imagem com o filtro Sobel aplicado à direita, como desmonstrado na figura \autoref{fig:gatoNormalESobel}.

\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Resultado do Filtro Sobel}
	\figuraborda{width=0.95\textwidth}{assets/cat_regular_to_sobel.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: (Original) https://www.flickr.com/photos/evapro/539719106}
	\label{fig:gatoNormalESobel}
\end{figure}

\subsection{CRC em Detalhes}
...
A \autoref{fig:crcDemo1} demonstra estado inicial do cálculo do CRC de cada linha. Na janela superior direita estão os conteúdos da memória do vetor de CRCs e na parte inferior os dados de pixel da imagem output.

Na \autoref{fig:crcDemo2} é demonstrado a imagem após ter as linhas filtradas e seus valores de checagem calculados. É introduzido uma corrupção na primeira linha do output (para facilitar visualização) na \autoref{fig:crcDemo3}.

Após a inserção, é realizado a checagem dos valores CRC com suas linhas correspondentes, os valores são modificados e viram um vetor que serve como uma máscara booleana para decidir quais linhas necessitam ser recomputadas, linhas marcadas com 0 indicam necessidade de recomputação pois a checagem falhou. A \autoref{fig:crcDemo4} demonstra que a linha que sofreu um upset é corretamente identificada. Cores indicam a relação do valor e sua região de memória.

\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Estado Inicial}
	\figuraborda{width=1.0\textwidth}{assets/crc_demo_1.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:crcDemo1}
\end{figure}
\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Convolução aplicada e CRCs de cada linha calculados}
	\figuraborda{width=1.0\textwidth}{assets/crc_demo_2.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:crcDemo2}
\end{figure}
\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Inserção de um valor upset}
	\figuraborda{width=1.0\textwidth}{assets/crc_demo_3.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:crcDemo3}
\end{figure}
\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Comparação dos CRCs resulta em máscara de execução}
	\figuraborda{width=0.95\textwidth}{assets/crc_demo_4.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:crcDemo4}
\end{figure}
...
\subsection{Consenso (TMR) em Detalhes}
...
Na \autoref{fig:tmrDemo1} observa-se o estado inicial, as 3 tarefas replicadas são criadas e aguardam seus semáforos binários para iniciar um novo ciclo de trabalho.

Uma corrupção de um NOT subtraído por 1 é realizada na segunda tarefa (índice 1), a discrepância entre o conteúdo das linhas das tarefas é demonstrado em \autoref{fig:tmrDemo2}.

Na seguinte iteração (\autoref{fig:tmrDemo3}), a função de consenso detecta e mascara o erro, selecionado o resultado majoritário da terceira tarefa (índice 2) que está em concordância com a primeira.

\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Estado inicial - 3 tarefas}
	\figuraborda{width=0.95\textwidth}{assets/tmr_demo_1.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:tmrDemo1}
\end{figure}
...
\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Upset inserido na segunda tarefa}
	\figuraborda{width=0.95\textwidth}{assets/tmr_demo_2.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:tmrDemo2}
\end{figure}
...
\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Consenso majoritário seleciona opção que mascara o upset}
	\figuraborda{width=0.95\textwidth}{assets/tmr_demo_3.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:tmrDemo3}
\end{figure}
...
Após repetir o processo de injeção da \autoref{sec:execucao} para as combinações de técnicas, foi possível coletar os dados representados no \autoref{tab:resSemInj}. O pico de memória da arena para o armazenamento das tarefas é denominado $M_{task}$, o pico da arena de memória adicional é denominado $M_{extra}$. As variáveis $T_{total}$ e $T_{linha}$ representam o tempo total de execução e o tempo médio por linha, respectivamente. A variável $D$ indica o número de detecções de erros bem-sucedidas. Os valores da execução sem técnicas serão utilizados como base e os outros valores serão expressos em termos de múltiplos, demonstrando o overhead adicionado.
...
\begin{quadro}[H]
	...
	\begin{tabular}{|l|c|c|c|c|c|c|}
		...
		N/A & 1542ms & 12ms & 2768B & 374B & 0 & OK \\ \hline
		CRC32 & 1.005 & 1.0 & 1.0 & 2.289 & 0 & OK \\ \hline
		Reexec & 3.003 & 3.167 & 1.0 & 2.872 & 0 & OK \\ \hline
		Reexec + CRC32 & 3.006 & 3.167 & 1.0 & 4.16 & 0 & OK \\ \hline
		TMR & 3.012 & 3.167 & 3.012 & 3.011 & 0 & OK \\ \hline
		TMR + CRC32 & 3.016 & 3.167 & 3.012 & 4.299 & 0 & OK \\ \hline
...
Injetando faltas nos filtros e no output como descrito no \autoref{cap:proj}, foi possível observar o comportamento das técnicas na presença de faltas que não envolvam corrupção de seu stack frame.

\begin{quadro}[H]
	\centering
	\caption{Resultados com Injeção em variáveis fixas}
	\begin{tabular}{|p{0.095\textwidth}|c|c|c|c|c|c|p{0.175\textwidth}|}
		\hline
		\rowcolor[HTML]{C0C0C0}
		\textbf{Técnicas} & \textbf{$T_{total}$} & \textbf{$T_{linha}$} & \textbf{$M_{task}$} & \textbf{$M_{extra}$} & \textbf{$D$} & \textbf{Resultado} & \textbf{Observação} \\
		\hline

		N/A            & 1544ms & 12ms  & 2768B & 374B  & 0 & Corrupção & N/A \\ \hline
		CRC32          & 1.014  & 1.0   & 1.0   & 2.289 & 1 & OK        & Corrupção detectada no output apenas \\ \hline
		Reexec         & 3.001  & 3.167 & 1.0   & 2.872 & 1 & Corrupção & Mascarou corrupção na linha, mas não no output \\ \hline
		Reexec + CRC32 & 3.028  & 3.167 & 1.0   & 4.16  & 2 & OK        & Corrupção mascarada e output recomputado \\ \hline
		TMR            & 3.01   & 3.167 & 3.012 & 3.011 & 1 & Corrupção & Mascarou corrupção na linha, mas não no output \\ \hline
		TMR + CRC32    & 3.014  & 3.167 & 3.012 & 4.299 & 2 & OK        & Corrupção mascarada e output recomputado \\ \hline
...
No \autoref{tab:resInjecaoFixa} é possível observar um padrão, a correção de erro ao final (output) só pode ser concluída no casos em que se possui a presença de um algoritmo de checagem, as técnicas de TMR e Reexecução conseguiram com sucesso mascarar a falta local de uma linha da corrupção com um uma função de consenso majoritário simples, porém uma vez que o valor já tenha sido calculado não é mais possível validar sua integridade. Para cada técnica foi escolhida uma variável do stack frame e aplicado um upset a partir de um offset e um tamanho. A notação $V+O:N$ indica que foi aplicado uma corrupção de tamanho $N$ a partir do endereço da variável $V$ deslocado por $O$ bytes. Unidades absolutas foram utilizadas para os casos em que a não foi possível alcançar um valor base.

\begin{quadro}[H]
	\centering
	\caption{Resultados com Injeção no Stack Frame}
	\begin{tabular}{|p{0.09\textwidth}|c|c|c|c|c|c|p{0.175\textwidth}|p{0.055\textwidth}|}
		\hline
		\rowcolor[HTML]{C0C0C0}
		\textbf{Técnicas} & \textbf{$T_{total}$} & \textbf{$T_{linha}$} & \textbf{$M_{task}$} & \textbf{$M_{extra}$} & \textbf{$D$} & \textbf{Resultado} & \textbf{Observação} & \textbf{Var} \\
		\hline

		N/A            & >1100ms & 60ms & 1.0   & 1.0   & 1 & Erro & Prazo de linha expirada               & row 0:8 \\ \hline
		CRC32          & >1100ms & 60ms & 1.0   & 2.289 & 1 & Erro & Prazo de linha expirada               & row 0:8 \\ \hline
		Reexec         & <100ms  & 0ms  & 1.0   & 2.872 & 1 & Erro & Assert: row.width != output.width     & output rows 4:12 \\ \hline
		Reexec + CRC32 & 2265ms  & 38ms & 1.0   & 4.16  & 1 & Erro & Hard Fault do FreeRTOS                & output rows 4:12 \\ \hline
		TMR            & 4625ms  & 38ms & 3.012 & 3.011 & 0 & OK   & Evitou corrupção através de indireção & tmr0 0:4 \\ \hline
		TMR + CRC32    & 4628ms  & 38ms & 3.012 & 4.299 & 0 & OK   & Evitou corrupção através de indireção & tmr0 0:4 \\ \hline

		% N/A	         & ~1100ms & 60ms & 2768B & 374B  & 1 & Erro & Prazo de linha expirada               & row 0:8 \\ \hline
		% CRC32          & ~1100ms & 60ms & 2768B & 856B  & 1 & Erro & Prazo de linha expirada               & row 0:8 \\ \hline
		% Reexec         & <100ms  & 0ms  & 2768B & 1074B & 1 & Erro & Assert: row.width != output.width     & output\_rows 4:12 \\ \hline
		% Reexec + CRC32 & 2265ms  & 38ms & 2768B & 1556B & 1 & Erro & Hard Fault do FreeRTOS                & output\_rows 4:12 \\ \hline
		% TMR            & 4625ms  & 38ms & 8336B & 1126B & 0 & OK   & Evitou corrupção através de indireção & tmr0 0:4 \\ \hline
		% TMR + CRC32    & 4628ms  & 38ms & 8336B & 1608B & 0 & OK   & Evitou corrupção através de indireção & tmr0 0:4 \\ \hline

	\end{tabular}
	\label{tab:resInjecaoStackFrame}
\end{quadro}

% TODO botar isso num apendice?
\section{Impacto de faltas no Output}

Foram selecionadas execuções que resultam em um output que possa ser visualizado para demonstrar o efeito dos upsets no resultado final. A \autoref{fig:gatoNormalESobel} demonstra o output desejado, com a imagem de um gato com um filtro Sobel aplicado. A \autoref{fig:catSobelKernelUpset} mostra o impacto de um upset causado no filtro (kernel) da convolução. Na \autoref{fig:catSobelSubtleUpset} a corrupção é bem mais sutil, manifestando-se como uma pequena linha horizontal em $y = 6$. Por fim, a figura \autoref{fig:catSobelInterrupted} demonstra o efeito da convolução sendo interrompida devido à um assert, deixando o resultado incompleto.

\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Imagem com o parâmetro de filtro corrompido}
	\figuraborda{width=0.3\textwidth}{assets/cat_sobel_kernel_upset.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:catSobelKernelUpset}
\end{figure}

\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Imagem com Corrupção sutil}
	\figuraborda{width=0.3\textwidth}{assets/cat_sobel_subtle_upset.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:catSobelSubtleUpset}
\end{figure}

\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
	\caption{Imagem com filtro Sobel interrompido}
	\figuraborda{width=0.3\textwidth}{assets/cat_sobel_interrupted.png }
	\captionsetup{justification=raggedright}
	\caption*{Fonte: Elaborada pelo autor}
	\label{fig:catSobelInterrupted}
\end{figure}


\section{Verificação}

No \autoref{tab:verif} são verificados os requisitos funcionais estabelecidos na análise de requisitos da \autoref{sec:req}. A validação de cada requisito foi realizada através de métodos específicos: \textbf{RF01} através da execução dos experimentos, \textbf{RF02} pela interface de configuração desenvolvida, \textbf{RF03} pelos testes sem injeção de faltas, \textbf{RF04} pela comparação de dependabilidade entre técnicas, e \textbf{RF05} pela implementação do sistema de monitoramento. Todos os requisitos funcionais foram satisfatoriamente atendidos.

\begin{quadro}[H]
	\centering
	\caption{Validação dos Requisitos funcionais}
	\begin{tabular}{|p{0.08\textwidth}|p{0.40\textwidth}|p{0.45\textwidth}|}
		\hline
		\rowcolor[HTML]{C0C0C0}
		...
		\textbf{RF03} & Cumprimento do prazo estipulado no momento de criação da tarefa caso não exista presença de faltas & Validado com os testes sem injeção de falta \\
		\hline
		
		\textbf{RF04} & Dependabilidade superior à versão do sistema sem técnicas & O sistema foi capaz de mascarar algumas faltas e detectar outras. Portanto possui uma dependabilidade superior \\
		\hline
		
		\textbf{RF05} & Monitoramento do número de faltas detectadas e violações de prazos & Implementado em \texttt{DeadlineWatcher} em conjunto com o IWDG. Foi capaz de detectar violações \\
		\hline
	\end{tabular}
	\label{tab:verif}
\end{quadro}

Os requisitos não funcionais também foram analisados conforme o \autoref{tab:verirnf}. E foram em sua maioria satisfeitos, com ressalvas apenas no \textbf{RNF05} devido à uma incompatibilidade de licença da biblioteca fornecida pelo fabricante.
