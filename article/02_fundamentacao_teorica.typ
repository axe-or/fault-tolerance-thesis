...
Dado um grafo direcionado não ponderado acíclico, será utilizado a notação $T_X [N]$, onde $X$ é o número identificador da tarefa, e $N$ corresponde à sua $N$-ésima re-execução, por exemplo $T_2 [1]$ indica a primeira execução da tarefa $T_2$, enquanto $T_1 [3]$ indica a terceira reexecução da tarefa $T_1$. Uma notação similar será utilizada para mensagens entre tarefas, $m_X [N]$, mensagens, assim como tarefas, estão sujeitas à falhas e overheads de detecção, mas ao invés de re-execução, mensagens são re-enviadas ou restauradas no caso algoritmos de recuperação de erro estejam disponíveis. @SchedFTWithSoftAndHardConstraints
